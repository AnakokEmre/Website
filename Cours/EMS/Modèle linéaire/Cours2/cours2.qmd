...
::: {.fragment .fade-transparent}
  * Y'a t'il un lien entre le poids du poisson et sa largeur après avoir pris en compte sa longueur et son épaisseur ? 
...
- $x_{2,i}$ la mesure de la largeur du poisson $i$.
- $x_{3,i}$ la mesure de l'épaisseur du poisson $i$.
...
$$\widehat{\theta}_k \sim \mathcal{N}\left(\theta_k,\sigma^2c_{kk}\right)\text{ avec } c_{kk} = (X^tX)^{-1}_{k,k}$$
...
$$\begin{align}SCR(M_{p+1})&=\sum_{i=1}^n E_i^2 \\ &=\sum_{i=1}^n\left(Y_i-(B+\sum_{j=1}^p A_jx_{i,j})\right)^2\end{align}$$
...
&=\frac{1}{n-p-1}\sum_{i=1}^n E_i^2\end{align}$$

$$\begin{align}{S^2}_{(M_{p+1})}&=\frac{\displaystyle\sum_{i=1}^n\left(Y_i-(B+\sum_{j=1}^p A_jx_{i,j})\right)^2}{n-p-1}\\
...
- Tester si un ensemble de $\g{q}$ variables explicatives ne suffit pas à expliquer $Y$, avec $\g{q}<\o{p}$.
...
- [**Autre formulation :**]{.upc}  
 ...
 $$H_0\ :\ \text{modèle}\ \g{M_{q+1}}\ :$$
 
 $$Y_i=\beta+\sum_{j=1}^{\g{q}}\alpha_j x_{i,j}+E_i,\quad E_i\overset{i.i.d.}{\sim}{\cal N}(0,\sigma^2)$$ contre l'alternative $$H_1\ :\ \text{modèle}\ \o{M_{p+1}}:$$
...
$$\begin{align}
\hat{\theta}^{(\o{p})} &= (B^{(\o{p})},A_1^{(\o{p})},\dots,A_{\o{p}}^{(\o{p})})^t\\
&= \left((X^{(\o{p})})^t\,X^{(\o{p})}\right)^{-1}\,\!(X^{(\o{p})})^t\,Y\\
\hat{\theta}^{(\g{q})}  & =  (B^{(\g{q})},A_1^{(\g{q})},\dots,A_{\g{q}}^{(\g{q})})^t\\
&=\left((X^{(\g{q})})^t\,X^{(\g{q})}\right)^{-1}\,\!(X^{(\g{q})})^t\,Y\ 
\end{align}$$
...
$$\begin{align}E_i(\o{M_{p+1}})&=Y_i-\widehat{Y_i}(\o{M_{p+1}})\\
&=Y_i-(B^{(\o{p})}+\sum_{j=1}^{\o{p}} A^{(\o{p})}_jx_{i,j})\\
E_i(\g{M_{q+1}})&=Y_i-\widehat{Y_i}(\g{M_{q+1}})\\
&=Y_i-(B^{(\g{q})}+\sum_{j=1}^{\g{q}} A^{(\g{q})}_jx_{i,j})
\end{align}$$
...
$$\begin{align}SCR(\o{M_{p+1}}) &= \sum_{i=1}^{n}\left(Y_i-B^{(\o{p})}-\sum_{j=1}^{p}A^{(\o{p})}_j\,x_{i,j}\right)^2\\ &=\ ||Y - X^{(\o{p})}\hat{\theta}^{(\o{p})}||^2\end{align}$$ 

$$\begin{align}SCR(\g{M_{q+1}})& = \sum_{i=1}^{n}\left(Y_i-B^{(\g{q})}-\sum_{j=1}^{\g{q}}A^{(\g{q})}_j\,x_{i,j}\right)^2\\& =\ ||Y - X^{(\g{q})}\hat{\theta}^{(\g{q})}||^2\end{align}$$
...
$$\begin{align}&\frac{\left(SCR(\g{M_{q+1}}) - SCR(\o{M_{p+1}})\right)\Bigl/(\o{p}-\g{q})}{SCR(\o{M_{p+1}})\Bigl/~(n\o{-p-1})}\\ &\sim_{H_0}^{}~\mathcal{F}(\o{p}-\g{q}\,,\,n\o{-p-1})\end{align}$$
...
$$\begin{align}T_n&=\frac{(SCR(\g{M_{q+1}})-SCR(\o{M_{p+1}}))/(\g{p}-\o{q})}{SCR(\o{M_{p+1}})/(n\o{-p-1})}\\
&\sim_{H_0} \mathcal{F}(\o{p}-\g{q},n\o{-p-1})\end{align}$$
:::
...
$$\begin{align}p_c&=\mathbb{P}_{H_0}(T_n > t_n)\\&=\mathbb{P}(\mathcal{F}(p-q,n-p-1)>t_n)\end{align}$$
:::
:::
...
::: {.scroll-container style="overflow-y: scroll;"}
...
\g{M_{q+1}} &&&&&&\\
\hline \text{Modèle} & n\o{-p-1} & SCR(\o{M_{p+1}}) & \g{p}-\o{q} & SCE=SCR(\g{M_{q+1}}) & t_n=&\mathbb{P}(\mathcal{F}(\o{p}-\g{q},n\o{-p-1})>t_n)\\
\o{M_{p+1}} &&&& - SCR(\o{M_{p+1}}) &\frac{SCE/(\o{p}-\g{q})}{SCR(\o{M_{p+1}})/(n\o{-p-1})} &\\
\hline
\end{array}$$
